...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None):
        ...
        # self.gabor_layer = GaborConv2D(1, embed_dim // 2, kernel_size=3, stride=2, padding=1)
        # self.Log_layer = LoGConv2D(1, embed_dim // 2, kernel_size=3, stride=2, padding=1)
        #
        ...
        #                         nn.Conv2d(embed_dim, embed_dim, kernel_size=3, stride=2, padding=1), nn.LeakyReLU()
        #                           )
    ...
    def forward(self, x):
        ...
        # x=torch.cat([x, self.gabor_layer(x)], dim=1)
        # x = self.proj(x)
        # x= torch.cat([self.gabor_layer(x), self.Log_layer(x)], dim=1)
        x=self.proj(x)
...
class ConvSwinTransformerSys(nn.Module):
    ...
    def forward(self, x):
        # print(self.patch_embed.gabor_layer.theta)
